...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{results/figures/rgb_vs_fft_pair.png}
  \caption{%
    \textbf{RGB vs.\ FFT spectrum comparison.}
    Each row pairs a face image (left) with its log-magnitude FFT spectrum
    (right): a StyleGAN2-generated \emph{fake} (top) and a \emph{real} face
    (bottom).
    Both spectra concentrate energy along the horizontal and vertical
    frequency axes; this cross-hair pattern — a periodic artefact of
    transposed-convolution upsampling — is most pronounced in the
