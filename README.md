...
Grad-CAM hooks are attached to the last convolutional layer of each ResNet18 branch.
...
│   ├── metrics/                  # Per-model JSON metrics + PNGs (committed)
...
│
├── poster/
│   └── poster.html               # Academic poster (HTML, print to PDF)
│
├── requirements.txt
├── .gitignore
└── README.md
...
```bash
git clone https://github.com/your-username/deepfake-frequency-detection.git
...
results/
├── figures/
│   ├── sample_images.png              ← Real vs Fake face grid
│   ├── rgb_vs_fft_comparison.png      ← Spatial vs frequency domain
│   ├── training_rgb.png               ← Loss & accuracy (RGB)
│   ├── training_fft.png               ← Loss & accuracy (FFT)
│   ├── training_hybrid.png            ← Loss & accuracy (Hybrid)
│   ├── confusion_matrix_rgb.png       ← Per-model confusion matrices
│   ├── confusion_matrix_fft.png
│   ├── confusion_matrix_hybrid.png
│   ├── roc_rgb.png                    ← Per-model ROC curves
│   ├── roc_fft.png
│   ├── roc_hybrid.png
│   ├── roc_combined.png               ← All three overlaid
│   ├── metrics_comparison.png         ← Grouped bar chart (5 metrics)
│   ├── gradcam_rgb_correct.png        ← Grad-CAM correct predictions (RGB)
│   ├── gradcam_rgb_wrong.png          ← Grad-CAM failure cases (RGB)
│   ├── gradcam_hybrid_correct.png     ← Grad-CAM correct (Hybrid)
│   ├── gradcam_hybrid_wrong.png       ← Grad-CAM failures (Hybrid)
│   ├── failure_cases_rgb.png          ← Plain failure gallery
│   ├── failure_cases_fft.png
│   └── failure_cases_hybrid.png
│
├── metrics/
